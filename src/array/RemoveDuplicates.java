...
public class RemoveDuplicates {
    ...
    public int removeDuplicates(int[] nums) {
        ...
        // 'prev' is the index of the last unique element found
        int prev = 0;

        for (int i = 1; i < nums.length; i++) {
            // If we find a new unique value
            if (nums[prev] != nums[i]) {
                prev++;           // Move the unique pointer forward
                nums[prev] = nums[i]; // Update that slot with the new value
            }
        }

        // Return the count (index + 1)
        return prev + 1;
    }
    ...
    public static void main(String[] args) {
        ...
        // Example 1
        int[] nums1 = {1, 1, 2};
        runExample(solution, nums1, 1);

        // Example 2
        int[] nums2 = {0, 0, 1, 1, 1, 2, 2, 3, 3, 4};
        runExample(solution, nums2, 2);
    }

    // Helper method to run and print results
    private static void runExample(RemoveDuplicates sol, int[] nums, int exampleNum) {
